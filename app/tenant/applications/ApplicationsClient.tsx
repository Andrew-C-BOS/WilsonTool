...
function Badge({ children, tone = "gray" }: { children: React.ReactNode; tone?: "gray"|"blue"|"amber"|"violet"|"emerald"|"rose" }) {
  ...
  return <span className={clsx("inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ring-1 ring-inset", map[tone])}>{children}</span>;
}
...
function Modal({ open, title, onClose, children }: { open: boolean; title: string; onClose: () => void; children: React.ReactNode }) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div role="dialog" aria-modal="true" className="absolute left-1/2 top-16 -translate-x-1/2 w-[92%] max-w-md rounded-xl bg-white shadow-xl ring-1 ring-gray-200">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <h3 className="text-base font-semibold text-gray-900">{title}</h3>
          <button onClick={onClose} className="rounded-md border border-gray-300 px-2 py-1 text-xs text-gray-700 hover:bg-gray-50">Close</button>
        </div>
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-3xl px-4 sm:px-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <Link
            ...
            className="rounded-lg bg-blue-600 text-white font-medium text-center px-4 py-3 hover:bg-blue-700"
          >
            Start new application
          </Link>
          <button
            onClick={() => setJoinOpen(true)}
            className="rounded-lg border border-gray-300 bg-white text-gray-900 font-medium px-4 py-3 hover:bg-gray-50"
          >
            Join with a code
          </button>
          {last ? (
            <Link
              ...
              className="rounded-lg border border-gray-300 bg-white text-gray-900 font-medium text-center px-4 py-3 hover:bg-gray-50"
            >
              Resume last application
            </Link>
          ) : (
            <div className="rounded-lg border border-gray-200 bg-gray-50 text-gray-500 text-center px-4 py-3">
              No drafts yet
            </div>
          )}
        </div>
              ...
              return (
                <div key={a.id} className="rounded-xl border border-gray-200 bg-white p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="text-sm font-semibold text-gray-900">{a.formName}</div>
                    ...
                    <div className="text-right">
                      <Link
                        ...
                        className="rounded-md bg-gray-900 text-white text-xs font-medium px-3 py-1.5 hover:bg-black"
                      >
                        Open
                      </Link>
                    </div>
                  ...
                  <div className="mt-3">
                    ...
                    <div className="mt-3 grid grid-cols-3 gap-2">
                      <div className="rounded-md border border-gray-200 bg-gray-50 p-2 text-center">
                        <div className="text-[11px] text-gray-600">My tasks</div>
                        <div className="text-sm font-semibold text-gray-900">{a.tasks?.myIncomplete ?? 0}</div>
                      </div>
                      <div className="rounded-md border border-gray-200 bg-gray-50 p-2 text-center">
                        <div className="text-[11px] text-gray-600">Household</div>
                        <div className="text-sm font-semibold text-gray-900">{a.tasks?.householdIncomplete ?? 0}</div>
                      </div>
                      <div className="rounded-md border border-gray-200 bg-gray-50 p-2 text-center">
                        <div className="text-[11px] text-gray-600">Missing docs</div>
                        <div className="text-sm font-semibold text-gray-900">{a.tasks?.missingDocs ?? 0}</div>
                      </div>
                    </div>
                  </div>
      ...
      <Modal open={joinOpen} title="Join an existing application" onClose={() => setJoinOpen(false)}>
        <div className="space-y-3">
          <p className="text-sm text-gray-700">Enter the invite code you received, we’ll attach you to the right household,</p>
          <input
            value={joinCode}
          ...
          <div className="flex items-center justify-end gap-2">
            <button onClick={() => setJoinOpen(false)} className="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm">Cancel</button>
            <button onClick={onJoin} className="rounded-md bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-700">Continue</button>
          </div>
      ...
      <Modal open={!!chatOpen} title="Household chat" onClose={() => setChatOpen(null)}>
        <div className="space-y-3">
          ...
          <div>
            ...
            <textarea className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm" rows={3} placeholder="Thanks for the invite, I’ll upload my ID tonight," />
          </div>
          <div className="flex items-center justify-end gap-2">
            <button onClick={() => setToast("Message queued, realtime chat coming soon,")} className="rounded-md bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-700">
              Send
            </button>
          </div>
        </div>
