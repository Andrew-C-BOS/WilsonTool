    ...
    <main className="min-h-[calc(100vh-4rem)] bg-[#e6edf1]">
      ...
      <div className="mx-auto max-w-3xl p-4 sm:px-6">
        <header className="rounded-3xl bg-gradient-to-r from-indigo-50 via-sky-50 to-rose-50 p-6 shadow-sm ring-1 ring-indigo-100/60">
          ...
          {/* Primary CTAs: bolder when user has no applications yet */}
          <div
            className={clsx(
              ...
              return (
                <div key={a.id} className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    {/* Action row: utilities left, progress right */}
                    <div className="flex w-full justify-between gap-2 sm:gap-3 sm:text-right">
                      {isLeaseActive(a.status) ? (
                        <>
                          {/* Lease-active: utilities empty, single primary CTA on the right */}
                          <div className="flex flex-wrap gap-2" />
                          <div className="flex flex-wrap gap-2 justify-end">
                            <Link
                              href={`/tenant/lease?app=${encodeURIComponent(a.id)}`}
                              className="inline-flex justify-center rounded-md bg-emerald-600 px-3 py-2 text-sm font-medium text-white hover:bg-emerald-700 active:opacity-90"
                        ...
                        <>
                          {/* Left: utility actions (Chat, Withdraw) */}
                          <div className="flex flex-wrap gap-1.5">
                            {/* Chat – neutral utility */}
                            <button
                              ...
                              className={clsx(
                                "inline-flex items-center rounded-full border border-gray-200 bg-white px-2.5 py-1 text-[11px] font-medium text-gray-700 hover:bg-gray-50 active:opacity-90",
                            ...
                            {/* Withdraw – destructive, but always secondary */}
                            {canWithdraw(a.status) && (
                              <button
                                disabled={withdrawBusyId === a.id}
                                onClick={() => onWithdraw(a)}
                                className={clsx(
                                  "inline-flex items-center rounded-full border border-rose-200 bg-white px-2.5 py-1 text-[11px] font-medium text-rose-700 hover:bg-rose-50 active:opacity-90",
                          ...
                          {/* Right: progress actions (Open/Review, Payments, Go to lease) */}
                          <div className="flex flex-wrap gap-2 justify-end">
                            {showPayments ? (
                              <>
                                {/* Secondary: Review */}
                                <button
                                  ...
                                  onClick={async () => {
                                    ...
                                    setOpenBusyId(a.id);
                                    try {
                                      await goToApply(a.formId, a.id);
                                    } finally {
                                      setOpenBusyId(null);
                                    }
                                  }}
                                  className={clsx(
                                    "inline-flex justify-center rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-900 hover:bg-gray-50 active:opacity-90",
                                    isBusy && "opacity-60 cursor-not-allowed",
                                  )}
                                  title="Review application"
                                >
                                  {isBusy ? "Opening…" : "Review"}
                                </button>

                                {/* Primary: Payments (right-most) */}
                                <button
                                  disabled={payBusyId === a.id}
                                  onClick={() => onPayHold(a.id)}
                                  className={clsx(
                                    "inline-flex justify-center rounded-md border border-blue-500 bg-blue-600 px-3 py-2 text-sm font-medium text-white hover:bg-blue-700 active:opacity-90",
                              ...
                              <>
                                ...
                                {/* Primary: Go to lease hub (right-most) */}
                                <Link
                                  href={`/tenant/lease?app=${encodeURIComponent(a.id)}`}
                                  className="inline-flex justify-center rounded-md bg-emerald-600 px-3 py-2 text-sm font-medium text-white hover:bg-emerald-700 active:opacity-90"
                              ...
                              // Default: application itself is the main work surface
                              <button
                                disabled={isBusy}
                                onClick={async () => {
                                  if (isBusy) return;
                                ...
                                className={clsx(
                                  "inline-flex justify-center rounded-md bg-gray-900 px-3 py-2 text-sm font-medium text-white hover:bg-black active:opacity-90",
                                  isBusy && "opacity-60 cursor-not-allowed",
                                )}
                            ...
                            {/* If you ever re-enable signatures, they’d also live here on the right */}
                            {showSignButton(a.status) && (
                              <Link
                                href={`/tenant/lease?app=${encodeURIComponent(a.id)}`}
                                className="inline-flex justify-center rounded-md border border-emerald-300 bg-emerald-50 px-3 py-2 text-sm font-medium text-emerald-800 hover:bg-emerald-100 active:opacity-90"
