...
export default function InspectionRouter() {
  ...
  useEffect(() => {
    ...
    catch { /* Safari */ /* @ts-ignore */ mql.addListener(onChange); /* @ts-ignore */ return () => mql.removeListener(onChange); }
  }, []);

  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/tenant/inspection", { cache: "no-store" });
