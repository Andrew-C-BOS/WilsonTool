...
function badgeFor(status?: Status) {
  switch (status) {
    case "succeeded": return { tone: "emerald" as const, label: "Payment received" };
    case "processing": return { tone: "violet" as const, label: "Payment submitted" };
    case "created": return { tone: "amber" as const, label: "Awaiting confirmation" };
    case "failed": return { tone: "rose" as const, label: "Payment failed" };
    case "canceled": return { tone: "gray" as const, label: "Payment canceled" };
    case "returned": return { tone: "rose" as const, label: "Payment returned" };
    default: return { tone: "gray" as const, label: "Payment status" };
  }
}
function nextSteps(status?: Status, kind?: string, rails?: string) {
  ...
  switch (status) {
    case "succeeded":
      return [
        "Funds have cleared.",
        k === "deposit" ? "Your deposit was received; we’ll issue the escrow disclosure." : "Your lease balance was funded.",
...
export default function PaymentsResultPage() {
  ...
  const key = sp.get("key") || ""; // Stripe payment code (PI id) or idempotency key
  ...
  const isTerminal = (s?: Status) => s === "succeeded" || s === "failed" || s === "canceled" || s === "returned";
  ...
  async function loadOnce() {
    ...
    try {
      const res = await fetch(`/api/tenant/payments/result?appId=${encodeURIComponent(appId)}&key=${encodeURIComponent(key)}`, {
        cache: "no-store",
      });
          ...
          <div className="mb-3">
            ...
            <p className="text-sm text-gray-700 mt-2">
              ...
              {steps.slice(1).map((s, i) => (
                <span key={i}><br />{s}</span>
              ))}
            </p>
          </div>

          <div className="mb-3 text-sm text-gray-700 space-y-1">
            <div>Amount: <strong>{money(data?.amountCents, data?.currency || "USD")}</strong></div>
            {data?.paymentIntentId && (<div>Payment code: <span className="font-mono text-gray-800">{data.paymentIntentId}</span></div>)}
            {data?.idempotencyKey && (<div>Request key: <span className="font-mono text-gray-700">{data.idempotencyKey}</span></div>)}
            {data?.updatedAt && (<div>Updated: <span className="text-gray-600">{new Date(data.updatedAt).toLocaleString()}</span></div>)}
          </div>
