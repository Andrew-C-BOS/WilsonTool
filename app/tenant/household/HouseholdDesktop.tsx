...
function Card({
  ...
  return (
    <div
      id={id}
      className={clsx("rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-100", border)}
    >
...
function GhostButton({
  ...
  const finalClassName = clsx(base, disabled && "opacity-60 cursor-not-allowed", className);

  return (
    <button className={finalClassName} onClick={onClick} disabled={disabled} title={title}>
      {children}
    </button>
  );
}
...
export default function HouseholdDesktop({
  ...
  // household name (optimistic)
  const [newName, setNewName] = useState(cluster.displayName || "");
  const [savingName, setSavingName] = useState(false);
  const [householdName, setHouseholdName] = useState(cluster.displayName || "");
  const [originalName, setOriginalName] = useState(cluster.displayName || "");
  ...
  // members (for optimistic updates, especially current user)
  const [members, setMembers] = useState(cluster.members);
  ...
  const headerName = householdName || "Untitled household";
  const hasIncomingInvites = incomingInvites.length > 0;

  // sync household + members if cluster changes from server
  useEffect(() => {
    setHouseholdName(cluster.displayName || "");
    setNewName(cluster.displayName || "");
    setOriginalName(cluster.displayName || "");
    setMembers(cluster.members);
  }, [cluster.displayName, cluster.members]);

  // initialize preferredName from current user's member row (once)
  useEffect(() => {
    if (preferredName.trim()) return;
    const email = (currentUserEmail || "").toLowerCase();
    if (!email) return;
    const me = cluster.members.find((m) => m.email.toLowerCase() === email);
    if (me?.name) {
      setPreferredName(me.name);
    }
  }, [currentUserEmail, cluster.members, preferredName]);
  ...
  async function saveHouseholdName() {
    ...
    const current = originalName.trim();
    ...
    // optimistic: update header immediately
  ...
  async function savePreferredName() {
    ...
    try {
      ...
      // optimistic update: reflect preferredName in the members list for the current user
      if (email) {
        setMembers((prev) =>
          prev.map((m) =>
  ...
  return (
    <main className="min-h-[calc(100vh-4rem)] bg-[#e6edf1]">
      <div className="mx-auto max-w-6xl px-6 py-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-gray-600">
                Your application is based on your household. Co-applicants and cosigners must be
                linked here so property managers see one clear group instead of separate
                submissions,
              </p>
            </div>
            ...
            <div>
              ...
              {isConfigure ? (
                <p className="mt-1 text-[11px] text-gray-500">
                  Your household will be complete once you set your household name (Rename
                  Household Section),
                </p>
              ) : (
                <p className="mt-1 text-[11px] text-gray-500">
          ...
          {isConfigure && (
            <div className="mt-5 grid gap-3 text-xs text-gray-600 sm:grid-cols-3">
              ...
              <div className="rounded-xl bg-white/70 px-3 py-3 shadow-sm">
                ...
                <p className="mt-1">
                  Set how your name appears to property managers, for example “Andrew C,” instead of
          ...
          >
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              ...
              <GhostButton
                ...
                disabled={savingName || !newName.trim() || newName.trim() === originalName.trim()}
        ...
        {/* Invite modal, join modals, toast (unchanged) */}
        {/* Invite modal (outgoing) */}
        <Modal
          open={inviteOpen}
          title="Share household invite"
        ...
        >
          <div className="space-y-3">
            ...
            {incomingLoading ? (
              <p className="text-sm text-gray-600">
                Looking for invites tied to your account…
              </p>
