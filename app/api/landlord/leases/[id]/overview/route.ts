...
function householdNameFromApp(app?: any | null): string | null {
  ...
  const mnames = members.map((m: any) => String(m?.name || "")).filter((s: string) => s.trim());
  if (mnames.length) return mnames.join(" & ");

  // fallback to email front part
  const anyEmail =
...
export async function GET(req: NextRequest, { params }: { params: { id?: string } }) {
  ...
  const leaseId = params?.id ? String(params.id) : "";
  if (!leaseId) return NextResponse.json({ ok: false, error: "missing_lease_id" }, { status: 400 });
  ...
  if (!lease) return NextResponse.json({ ok: false, error: "lease_not_found" }, { status: 404 });

  // Optional firm check if firmId passed
  if (firmIdParam && String(lease.firmId) !== String(firmIdParam)) {
    return NextResponse.json({ ok: false, error: "wrong_firm" }, { status: 403 });
  }
