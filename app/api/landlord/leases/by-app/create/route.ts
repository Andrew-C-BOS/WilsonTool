...
function addMonthsSameDay(ymd: string, months: number) {
  ...
  const dd = String(dt.getUTCDate() + 0).padStart(2, "0");
  return `${yy}-${mm}-${dd}`;
}
...
async function loadFirmDoc(db: any, firmId: string) {
  ...
  const a = await (db.collection("FirmDoc") as any).findOne(
    idEq("_id", firmId),
    { projection: proj }
  );
  if (a) return a;

  const b = await (db.collection("firms") as any)
    .findOne?.(idEq("_id", firmId), { projection: proj })
    .catch?.(() => null);
  return b || null;
}
...
async function resolveFirmForUser(
  ...
  const memCol = db.collection("firm_memberships") as any;
  const { or: userOr, debug: udbg } = buildUserMatch(user);

  const mems = await memCol
    .find({ active: true, $or: userOr }, { projection: { firmId: 1 } })
    .toArray();
  const membershipFirmIds = new Set<string>(
    mems.map((m: any) => String(m.firmId))
  );
  const hasMembership = (fid: string) =>
    membershipFirmIds.has(String(fid));

  if (opts.firmIdParam) {
    const ok = hasMembership(opts.firmIdParam);
    const firmDoc = ok
      ? await loadFirmDoc(db, String(opts.firmIdParam))
      : null;
    return {
      ok,
      ...
      debug: opts.debug
        ? {
            usedCollection: memCol.collectionName,
            membershipFirmIds: [...membershipFirmIds],
            ...udbg,
          }
        : undefined,
    };
  ...
  if (opts.appId) {
    const apps = db.collection("applications") as any;
    const forms = db.collection("application_forms") as any;
    const app = await apps.findOne(idEq("_id", opts.appId), {
      projection: { firmId: 1, formId: 1 },
    });
    if (app?.firmId) derivedFirmId = String(app.firmId);
    else if (app?.formId) {
      const form = await forms.findOne(idEq("_id", app.formId), {
        projection: { firmId: 1 },
      });
  ...
  if (derivedFirmId) {
    ...
    const firmDoc = ok
      ? await loadFirmDoc(db, derivedFirmId)
      : null;
    return {
      ok,
      firmId: ok && firmDoc ? toStringId(firmDoc._id) : null,
...
function buildDefaultChecklist(
  ...
  const baseDue =
    moveInYmd && isYYYYMMDD(moveInYmd)
      ? new Date(
          at10Z(moveInYmd).getTime() - 24 * 60 * 60 * 1000
        )
      : createdAt;

  const asIso = (d: Date | null) =>
  ...
  const items = [
    ...
    {
      key: "pay_upfront",
      label: "Pay remaining up-front charges",
      dueAt: createdAt,
      completedAt: null,
      notes: null,
    },
...
export async function POST(req: Request) {
  ...
  const resolved = await resolveFirmForUser(db, user, {
    firmIdParam: firmIdInput,
    appId,
    debug: debugMode,
  });
  ...
  const unitLeases = db.collection<any>("unit_leases");

  // Idempotency: if a lease for this app already exists, return it.
  const existing = await unitLeases.findOne(
    idEq("appId", appId)
  );
  if (existing) {
    return NextResponse.json({
  ...
  const building = {
    addressLine1: String(
      body?.building?.addressLine1 ||
        app?.building?.addressLine1 ||
        ""
    ),
    addressLine2:
      body?.building?.addressLine2 ??
      app?.building?.addressLine2 ??
      null,
    city: String(
      body?.building?.city || app?.building?.city || ""
    ),
    state: String(
      (body?.building?.state ||
        app?.building?.state ||
    ...
    postalCode: String(
      body?.building?.postalCode ||
        app?.building?.postalCode ||
        ""
    ),
    country: String(
      body?.building?.country || app?.building?.country || "US"
    ),
  };

  const unitNumber =
    String(
      body?.unitNumber ?? app?.unit?.unitNumber ?? ""
    ) || null;
  ...
  const moveInDate =
    String(
      body?.moveInDate ||
        plan?.startDate ||
        app?.protoLease?.moveInDate ||
        ""
    ) || null;

  // Compute moveOut if not provided
  let moveOutDate: string | null = body?.moveOutDate
    ? String(body.moveOutDate)
    : null;
  if (!moveOutDate && moveInDate && isYYYYMMDD(moveInDate)) {
    const term = Number(
      plan?.termMonths ??
        app?.protoLease?.termMonths ??
        12
    );
    if (Number.isFinite(term) && term > 0) {
      moveOutDate = addMonthsSameDay(moveInDate, term);
    }
  }
  ...
  if (!(monthlyRent > 0)) {
    return NextResponse.json(
      { ok: false, error: "invalid_rent" },
      { status: 400 }
    );
  }
  if (!isYYYYMMDD(moveInDate || "")) {
    return NextResponse.json(
      { ok: false, error: "invalid_move_in" },
      { status: 400 }
    );
  }
  ...
  await unitLeases.insertOne(leaseDoc as any);

  // ---------- Mark the application and push timeline events ----------
  const FINAL_STATUS =
    process.env.APP_FINAL_LEASE_STATUS || "leased";

  const timelineEvents: any[] = [
    {
      at: now,
      by: String(user._id ?? (user as any).id ?? "system"),
      event: "lease.created",
      meta: {
  ...
  await apps.updateOne(idEq("_id", appId), updateDoc);
