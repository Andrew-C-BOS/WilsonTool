...
export async function GET(
  ...
  const form = await forms.findOne(
    { _id: ObjectId.isValid(String(app.formId)) ? new ObjectId(String(app.formId)) : String(app.formId) },
    { projection: { firmId:1 } }
  );
  ...
  const uid = String((user as any)._id ?? (user as any).id ?? (user as any).userId ?? (user as any).email);
  ...
  const lease = await leases.findOne({ firmId, appId: String(app._id) }, { projection: { meta: 0 } });
