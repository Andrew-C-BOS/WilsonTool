...
function htmlShell(inner: string, title: string, opts?: { emailMode?: boolean }) {
  ...
  // no printScript, no print button
...
export async function GET(req: Request, ctx: any) {
  ...
  const premises = b
    ? `${b.addressLine1 ?? ""}${b.addressLine2 ? `, ${b.addressLine2}` : ""}, ${
        b.city ?? ""
      }, ${b.state ?? ""} ${b.postalCode ?? ""}${u?.unitNumber ? ` — Unit ${u.unitNumber}` : ""}`
    : "Premises";
  ...
  const rails = (
    payment.rails ?? payment.provider ?? payment?.meta?.rails ?? "—"
  ).toString();
  ...
  const depositISO = toISO(
    payment.succeededAt ?? payment.processingAt ?? payment.createdAt
  );
  ...
  // signature
  const sigMeta = (payment.meta?.receiptSignature ?? {}) as {
    name?: string;
    atISO?: string;
    imageUrl?: string;
  };
  ...
  const initialReceiptMissing =
    !landlord || !premises || !receivedOn || !amount || !sigName;
  ...
  <div class="box">
    ...
    <div class="grid">
      ...
      <div class="row"><span class="label">Deposit Date</span>${esc(
        depositDate || "—"
      )}</div>
