  ...
  return (
    <div className="fixed inset-0 z-50 md:hidden">
      ...
      <div
        role="dialog"
        aria-modal="true"
        className="absolute inset-x-0 bottom-0 top-auto w-full max-w-none rounded-t-2xl bg-white shadow-xl ring-1 ring-gray-200"
      >
...
const ENDPOINT = (cursor?: string) =>
  `/api/landlord/applications?limit=50${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ""}`;
...
function coerceToHouseholdUI(raw: any): Household | null {
  if (!raw) return null;
  const id =
    raw.id ?? raw.hhId ?? raw.householdId ?? raw.applicationGroupId ?? raw._id ?? null;
  if (!id) return null;

  const property =
    raw.property?.name ?? raw.propertyName ?? raw.property_title ?? raw.property ?? "";
  const unit =
    raw.unit?.label ?? raw.unitLabel ?? raw.unitNumber ?? raw.unit_name ?? raw.unit ?? "";
...
async function fetchHouseholds(cursor?: string): Promise<{
  rows: Household[];
  ...
  try {
    ...
    const nextCursor = (j as any).nextCursor ?? (j as any).cursor ?? (j as any).next ?? null;
...
export default function ApplicationsMobile() {
  // Optionally, detect admin via your /api/auth/me
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const { rows, nextCursor } = await fetchHouseholds();
      if (!cancelled) {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(
      (entries) => {
        const [e] = entries;
        if (e.isIntersecting && !busyMore) {
          void loadMore();
        }
      },
      { rootMargin: "120px 0px 240px 0px" }
    );
    io.observe(el);
  ...
  async function loadMore() {
    if (!cursor) return;
    setBusyMore(true);
    const { rows: nextRows, nextCursor } = await fetchHouseholds(cursor);
  ...
  function onReview(hh: Household) {
    setToast(`Open review for household in ${hh.property} ${hh.unit}, documents by member, notes next,`);
  }
  ...
  async function onFullAccept(hh: Household) {
    ...
    setToast(
      ok
        ? `Approval queued for ${hh.property} ${hh.unit}, lease generation up next,`
        : `Could not approve right now, please retry,`
    );
  }
  ...
  /* -------------- Render (mobile only) -------------- */
  return (
    <main className="md:hidden mx-auto w-full max-w-2xl px-3 pb-[max(1rem,env(safe-area-inset-bottom))]">
      {/* Sticky toolbar — scrollable chips, compact search, quick actions */}
      <div className="-mx-3 sticky top-0 z-30 border-b border-gray-200 bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/60">
        <div className="px-3 py-3 space-y-2">
          {/* Chips */}
          <div className="overflow-x-auto [-webkit-overflow-scrolling:touch]">
            <div className="inline-flex min-w-max rounded-lg border border-gray-300 bg-white p-0.5">
              {([
                { id: "all", label: "All" },
                { id: "new", label: "New" },
                ...
                <button
                  ...
                  onClick={() => setTab(t.id)}
                  className={clsx(
                    "px-3 py-2 text-sm rounded-md transition whitespace-nowrap",
      ...
      {/* List of households — cards, concise, readable */}
      <div className="mt-3 space-y-3">
        {loading ? (
          ...
          filtered.map((hh) => {
            ...
            return (
              <div key={hh.id} className="rounded-xl border border-gray-200 bg-white p-4">
                {/* Header row */}
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="mt-0.5 truncate text-xs text-gray-600">
                      {hh.property} · Unit {hh.unit}
                    </div>
                    <div className="mt-1 flex flex-wrap items-center gap-2">
                      <StatusChip status={hh.status} />
                      <span className="text-[11px] text-gray-500">
                        Submitted {formatDate(hh.submittedAt)}
                      </span>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <button
                      onClick={() => onReview(hh)}
                      className="rounded-md bg-gray-900 text-white text-xs font-medium px-3 py-1.5 active:scale-[0.99]"
                    >
                      Review
                    </button>
                  </div>
                ...
                {/* Members */}
                <div className="mt-3">
                  <div className="text-[11px] uppercase tracking-wide text-gray-500">Members</div>
                ...
                {/* Actions row — large tap targets, wrap as needed */}
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <button
                    onClick={() => setInviteOpen({ hhId: hh.id })}
                    className="rounded-md border border-gray-300 bg-white px-3 py-2 text-xs font-medium text-gray-900 active:scale-[0.99]"
                  >
                    Invite member
                  </button>
                  <button
                    onClick={() => onPrelimAccept(hh)}
                    className="rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-900 active:scale-[0.99]"
                  >
                    Prelim accept
                  </button>
                  <button
                    onClick={() => onFullAccept(hh)}
                    className="rounded-md border border-emerald-300 bg-emerald-50 px-3 py-2 text-xs font-medium text-emerald-900 active:scale-[0.99]"
                  >
                    Fully accept
                  </button>
                  <button
        ...
        <>
          ...
          <div className="flex justify-center my-3">
            <button
              disabled={busyMore}
              onClick={loadMore}
              className="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-900 disabled:opacity-60"
            >
              ...
              >
                <div>
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <Badge tone={m.role === "primary" ? "blue" : m.role === "cosigner" ? "violet" : "gray"}>
                      {m.role}
                    </Badge>
                    {m.state && (
                      <Badge
                        tone={m.state === "complete" ? "emerald" : m.state === "missing_docs" ? "amber" : "gray"}
                      >
                        {m.state.replace("_", " ")}
                      </Badge>
                    )}
                ...
                <div className="flex gap-2">
                  <button className="rounded-md border border-gray-300 bg-white px-2.5 py-1.5 text-xs">
                    Resend invite
                  </button>
                  <button className="rounded-md border border-gray-300 bg-white px-2.5 py-1.5 text-xs">
                    Request docs
