...
export async function GET(
  ...
  // tolerate string or ObjectId for _id
  const leaseIdLookup = ObjectId.isValid(id) ? new ObjectId(id) : id;

  const lease = await leases.findOne({ _id: leaseIdLookup as any });
  ...
  const membership = await fms.findOne(
    {
      firmId,
      active: true,
      $or: uidOid ? [{ userId: uid }, { userId: uidOid }] : [{ userId: uid }],
    },
    { projection: { _id: 1 } }
  );
...
export async function PATCH(
  ...
  if (moveInDate && !isIso(moveInDate))
    return NextResponse.json({ ok: false, error: "bad_move_in" }, { status: 400 });
  if (moveOutDate !== undefined && moveOutDate !== null && !isIso(moveOutDate))
    return NextResponse.json({ ok: false, error: "bad_move_out" }, { status: 400 });
  if (moveInDate && moveOutDate && moveOutDate <= moveInDate)
    return NextResponse.json({ ok: false, error: "move_out_before_in" }, { status: 400 });
